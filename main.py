...
def to_map(coordinates, countries):
    ...
    fg_c = folium.FeatureGroup(name="Countries")
    fg_pop = folium.FeatureGroup(name="Population")
    fg_pop.add_child(folium.GeoJson(data=open('world.json', 'r', encoding='utf-8-sig').read(), style_function=lambda x: {
    ...
    for films in coordinates:
        fg_film.add_child(folium.CircleMarker(location=[
...
def _main():
    ...
    print("Your map is My_Map.html in folder")
